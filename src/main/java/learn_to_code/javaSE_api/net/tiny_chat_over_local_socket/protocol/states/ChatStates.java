 ...
 * <li>{@code CLIENT_TERMINATED}. Client is terminated. Last state upon receiving which the server will drop client connection </li>
 * </ul>
 * <br>
 ...
 * <li>{@code TERMINATE_CLIENT} -> @{code CLIENT_TERMINATED}</li>
 * <li>@{code CLIENT_TERMINATED} -> No other state possible, as after receiving this state server drops client connection </li>
 * </ul>
 * Almost every state can go to itself. This mainly (but not always happens when 'normal' exception occurs, e.g. when user is trying to connect to a user, which is now offline)
 */
